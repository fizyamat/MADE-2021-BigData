...
def main(args):
    ...
    df = df.withColumn('ts', split_col.getItem(0))
    ...
    df2 = spark.sql("""
        ...
        FROM
            tmp
        GROUP BY
            window(to_timestamp(from_unixtime(ts)), '2 second', '1 second'), 
            CASE WHEN parse_url(url, 'HOST') LIKE '%.ru' THEN 'ru' ELSE 'not ru' END
        ORDER BY
            window,
            view DESC,
            zone
